...
import cn.usshenzhou.madparticle.mixin.BufferBuilderAccessor;
...
public class MadParticleBufferBuilder extends BufferBuilder {
    ...
    public void vertex(float pX, float pY, float pZ, float pRed, float pGreen, float pBlue, float pAlpha, float pTexU, float pTexV, int pOverlayUV, int pLightmapUV, float pNormalX, float pNormalY, float pNormalZ) {
        BufferBuilderAccessor accessor = (BufferBuilderAccessor) this;
        if (this.defaultColorSet) {
            throw new IllegalStateException();
        } else if (accessor.isFastFormat()) {
            this.putFloat(0, pX);
            this.putFloat(4, pY);
            this.putFloat(8, pZ);
            ...
            accessor.setNextElementByte(accessor.getNextElementByte() + i + 8);
            this.endVertex();
        } else {
